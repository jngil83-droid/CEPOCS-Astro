...
function renderProyectos(limit) {
  ...
  proyectosData.slice(0, limit).forEach((proyecto) => {
    const card = document.createElement('div');
    card.className = 'proyecto-card';
    card.innerHTML = `
      <div class="proyecto-header">
        <h3>${proyecto.titulo}</h3>
        <div class="proyecto-fecha">${proyecto.fecha} | ${proyecto.monto.toLocaleString('es-ES')} €</div>
      </div>
      <div class="proyecto-info">
        <p><strong>Localización:</strong> ${proyecto.localizacion}</p>
        <p><strong>Resumen:</strong> ${proyecto.resumen}</p>
        <div class="entidades-participantes"><strong>ENTIDADES PARTICIPANTES:</strong>${proyecto.entidades}</div>
      </div>`;
...
if (toggleProy) {
  toggleProy.addEventListener('click', () => {
...
window.addEventListener('scroll', () => {
  if (!header) return;
  header.classList.toggle('scroll-activo', window.scrollY > 60);
});